...
public class PtsOHLCV {
  ...
  public double[] getCloses(int beginIdx) {
    ...
    for (int i = beginIdx; i < PriceBars.getItemCount(); i++) {
      ohlcItem = (OHLCItem) PriceBars.getDataItem(i);
      _closes[closesIdx] = ohlcItem.getCloseValue();
      closesIdx++;
    }
    //setupArrays();
    return _closes;
  ...
  public double[] getHighs(int beginIdx) {
    int sizeNeeded = PriceBars.getItemCount() - beginIdx;
    double[] _highs = new double[sizeNeeded];
    OHLCItem ohlcItem;
    int closesIdx = 0;
    for (int i = beginIdx; i < PriceBars.getItemCount(); i++) {
      ohlcItem = (OHLCItem) PriceBars.getDataItem(i);
      _highs[closesIdx] = ohlcItem.getHighValue();
      closesIdx++;
    }
    //setupArrays();
    return _highs;
  ...
  public double[] getLows(int beginIdx) {
    int sizeNeeded = PriceBars.getItemCount() - beginIdx;
    double[] _lows = new double[sizeNeeded];
    OHLCItem ohlcItem;
    int closesIdx = 0;
    for (int i = beginIdx; i < PriceBars.getItemCount(); i++) {
      ohlcItem = (OHLCItem) PriceBars.getDataItem(i);
      _lows[closesIdx] = ohlcItem.getLowValue();
      closesIdx++;
    }
    //setupArrays();
    return _lows;
  ...
  public double[] getOpens(int beginIdx) {
    int sizeNeeded = PriceBars.getItemCount() - beginIdx;
    double[] _opens = new double[sizeNeeded];
    OHLCItem ohlcItem;
    int closesIdx = 0;
    for (int i = beginIdx; i < PriceBars.getItemCount(); i++) {
      ohlcItem = (OHLCItem) PriceBars.getDataItem(i);
      _opens[closesIdx] = ohlcItem.getOpenValue();
      closesIdx++;
    }
    //setupArrays();
    return _opens;
  ...
  public double[] getVolumes(int beginIdx) {
    int sizeNeeded = PriceBars.getItemCount() - beginIdx;
    double[] _volumes = new double[sizeNeeded];
    java.lang.Number n;
    int closesIdx = 0;
    for (int i = beginIdx; i < PriceBars.getItemCount(); i++) {
       _volumes[closesIdx] = Volume.getValue(i).doubleValue();
      closesIdx++;
    }
    //setupArrays();
    return _volumes;
