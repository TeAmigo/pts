...
public class PtsMouseKeyboardController {
  ...
  public void keyPressed(KeyEvent e) {
    ...
    if (e.isAltDown()) {  
      if (e.getKeyCode() == KeyEvent.VK_A) {
        ...
        chart.getTraderFrame().setLocationAndSize();
      }
    }
  }
  ...
  public void keyTyped(KeyEvent e) {
    if (!(e.isAltDown() || e.isControlDown())) { //Ctrl and Alt key handled elsewhere
      ...
      else if(keys[KeyEvent.VK_S]) { //TODO: RPC 7/5/11 7:02 PM Save the lines on the chart
        List dcs = panel.getCrosshair1().getDomainCrosshairs();
        for(int i = 0; i < dcs.size(); i++) {
          PtsCrosshair ch = (PtsCrosshair)dcs.get(i);
          Double val = ch.getValue();
          long dateLong = val.longValue();
          Timestamp tstamp = new Timestamp(dateLong);
          String dd = tstamp.toString();
          int k = 3;
        }
        List rcs = panel.getCrosshair1().getRangeCrosshairs();
        int j = 3;
      }
    }
  }
  ...
  /**
   * Process keyevents when the primary mouse button has been clicked
   * @param event 
   */
  public void chartMouseClicked(ChartMouseEvent event) {
    ...
    if (keys[KeyEvent.VK_L]) {
      ...
      panel.setupBuyCrosshair(chartX, subChartY, "BUY");
